...
static void solver_increase_iter(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('solver_increase_iter', hSolver)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  plhs[0] = mxCreateDoubleScalar(solver->increase_iter());
}
...
// Usage: caffe_('solver_update', hSolver)
static void solver_reset_diff(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('solver_update', hSolver)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  solver->ResetDiff();
...
static handler_registry handlers[] = {
  ...
  { "solver_increase_iter",          solver_increase_iter           },
  { "solver_step",                   solver_step                    },
  { "solver_reset_diff",             solver_reset_diff              },
  { "get_net",                       get_net                        },
